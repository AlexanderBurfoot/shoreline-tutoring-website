...
/** Get the SVG icon for a subject by slug, with optional size override */
export function getSubjectIcon(slug: string, size?: string): React.ReactNode {
    const icon = subjectIcons[slug];
    if (!icon || !size) return icon ?? null;

    // Clone with a custom font-size so the 1em-based SVGs scale
